...
class SimpleRLPlayer(Gen8EnvSinglePlayer):
    def __init__(self, *args, **kwargs):
        if 'username' not in kwargs:
            kwargs['username'] = f'Guest {str(uuid.uuid4())[:8]}'
...
async def main():
    ...
    try:
        # Create unique guest usernames
        player_id = str(uuid.uuid4())[:8]
        opponent_id = str(uuid.uuid4())[:8]

        # Create opponent first with guest authentication
        opponent = RandomPlayer(
            battle_format="gen8randombattle",
            username=f"Guest {opponent_id}",
        ...
        # Create the RL player with guest authentication
        env_player = SimpleRLPlayer(
            battle_format="gen8randombattle",
            opponent=opponent,
            start_challenging=True,
            username=f"Guest {player_id}",
            server_configuration={
                "authenticate": True,
                "authentication_url": "https://play.pokemonshowdown.com/action.php",
                "avatar": 1,
        ...
        if env_player is not None:
            await env_player.stop_challenging()
